...
> unaffected and stay in `.canon/`. Full-length ADR records remain in `DECISIONS.md`.
> Regenerated by /ai:migration.
...
Each entry is the capability's purpose and the invariants that must hold (its former
`openspec/specs/` contract, condensed).
...
Every build-vs-adopt decision that shaped the system, with its recorded rationale.
Type vocabulary: **Rent / Adopt / Extend / Fork / Build**. Full-length records live in
`DECISIONS.md` at the repository root (which survives OpenSpec's removal).

### Kernel core

| Concern                                 | Decision                                                                             | Why                                                                                                                                                                                                                                                            |
| --------------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Object identity grammar                 | Build — hand-written, one `re` validator                                             | No library owns `kind:namespace.object_name`; ~20 lines vs a dependency plus a grammar file. Rejected lark/parsimonious.                                                                                                                                       |
| Component registry                      | Build — dict + facet filters                                                         | Every mature candidate covers a different concern; none speaks the grammar. Rejected svcs (executes user code, collides with describes-never-executes), pluggy, stevedore.                                                                                     |
| Component resolution                    | Build — per-segment errors                                                           | Pure lookup; extending the in-repo exception hierarchy is the whole job. Rejected dishka/dependency-injector as a scope change into stated non-goals.                                                                                                          |
| Registry + decorator factories          | Build — existing kernel core                                                         | The grammar, closed kind set, per-segment errors, and validate-never-normalize are the differentiating value; best OSS match ≈40%.                                                                                                                             |
| TOML config parsing                     | Adopt — stdlib `tomllib`                                                             | Standard-format parsing is never hand-rolled; keeps the kernel zero-dependency.                                                                                                                                                                                |
| Dependency ordering                     | Adopt — stdlib `graphlib.TopologicalSorter`                                          | The hand-rolled dependency graph (~130 LOC) duplicated stdlib for zero differentiating value.                                                                                                                                                                  |
| Lifecycle hooks                         | Build — in-kernel                                                                    | An ordered callback list plus per-kind startup/shutdown; pluggy would trade the zero-dependency guarantee for ~5% feature use.                                                                                                                                 |
| Declarative config validation           | Build (minimal) — explicit checks over a closed key set                              | The reinvention risk was a general recursive schema engine, not this validation. Rejected msgspec (compiled extension every consumer inherits), jsonschema-as-runtime-dep (four transitive deps for a four-key contract).                                      |
| Component metadata validation           | Adopt + Build — `ty` statically; one identity assertion at the registration boundary | The data originates in author-written Python and crosses no trust boundary; a runtime validator would re-prove statically known facts on the hottest path.                                                                                                     |
| Async lifecycle execution               | Adopt — stdlib `asyncio` + `inspect`                                                 | The kernel only awaits sequentially — never schedules or cancels; `anyio` excluded by the zero-dependency invariant.                                                                                                                                           |
| Concurrency primitive                   | Adopt — stdlib `threading.Lock`                                                      | Mutual exclusion over two small critical sections; a lock is a language primitive, not a dependency.                                                                                                                                                           |
| Lifecycle state transitions             | Build — flags under one lock                                                         | `dependencies = []` makes a runtime state-machine library architecturally incompatible; three states, so the abstraction would exceed what it abstracts. Revisit trigger: more states or conditional transitions. Considered python-statemachine, transitions. |
| Failure-path logging                    | Adopt — stdlib `logging`, bridgeable but unbridged                                   | A library emits to a named logger; the application owns the pipeline. `NullHandler` on the `spoc` root, `getLogger(__name__)`, lazy `%s` + `exc_info=True`, so an app can bridge to OTel without SPOC depending on it. Rejected direct OTel and structlog.     |
| Reader draining during shutdown         | Adopt the host's — SPOC refuses to drain                                             | The capability exists one layer up and is guaranteed there (ASGI lifespan, gRPC `stop(grace)`); a framework-level drain would wait for readers the host promises are gone. Precedent: MS.DI shipped this and withdrew it over deadlock.                        |
| Transition membership (sync + async)    | Adopt — stdlib `contextvars`                                                         | One `ContextVar`'s isolation rules match both shapes: per-thread, and per-task via the copy taken at task creation. Rejected `threading.local()` (wrong on async).                                                                                             |
| Reentrancy determination                | Adopt — the same `contextvars` marker                                                | Reverses the earlier thread-identity mechanism, which misdiagnosed a racing async caller as reentrant.                                                                                                                                                         |
| Load-ordering guarantee                 | Extend — `graphlib` + explicit `(kind_depth, app_index)` sort key                    | Puts the guarantee where a reader of our own code can check it, instead of resting on a documented caveat about insertion order. networkx hard-rejected on `dependencies = []`.                                                                                |
| Cycle detection in the kind graph       | Adopt — `graphlib`, unchanged                                                        | An ordering key sorts a DAG but cannot notice the graph is not one.                                                                                                                                                                                            |
| Namespace-collision model               | Adopt + Build — Django's app-label contract; the check is ours                       | Derive-by-default / fail-loudly-on-contest / explicit override is Django's proven model. Improvement on the precedent: the error names both claimants.                                                                                                         |
| Explicit-namespace syntax               | Adopt + Build — Python's `as` convention (`"pkg.path as alias"`)                     | Reuses the language's own rebinding vocabulary; avoids overloading `:`, which already means "attribute".                                                                                                                                                       |
| Per-app metadata location               | Adopt — a central app list; refuse a colocated manifest                              | No third-party apps exist, so every fact a manifest would carry is already known where it would be read. Revisit when an app ships separately from the project.                                                                                                |
| Per-app framework-version compatibility | Rent — the packaging ecosystem's own check                                           | The project's dependency pin is the check, enforced by the installer before any code runs. Hand-rolling PEP 440 comparison is where a comparator is quietly wrong.                                                                                             |

### Packaging and distribution

| Concern                              | Decision                                                                              | Why                                                                                                                                                                                                                                                                       |
| ------------------------------------ | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Formats packaging                    | Adopt — one distribution; containment enforced by tests                               | Reverses the earlier uv-workspace split into `spoc-formats`: formats is a capability of the kernel, not its own bounded context; the two real defects the split fixed are pinned by tests instead. Standing owner mandate: exactly one PyPI distribution, never re-split. |
| Multi-platform execution of the gate | Rent — the CI platform in use; Linux + Windows + macOS × three Pythons, no exclusions | Infrastructure, so the hierarchy answers it; the cache module carries a darwin arm, so omitting macOS would ship a branch no gate executes.                                                                                                                               |
| Incrementing the declared version    | Adopt — `hatch version`                                                               | Hatchling already declares the version location, so adopting adds no configuration and cannot drift from what the build reads. Hard-rejected hatch-vcs/setuptools-scm — tag-derived versions would disable deprecation-lifecycle enforcement.                             |

### Formats subpackage

| Concern                             | Decision                          | Why                                                                                                                                                                                                                 |
| ----------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Multi-format loading and collection | Build (thin) over adopted parsers | anyconfig/dynaconf would become a dependency of every format including JSON, breaking the bare-install requirement.                                                                                                 |
| XML dict convention                 | Adopt — `xmltodict`               | Maintained, MIT, pure Python, no deps; its `force_list` callable form is precisely the hook declared repeating paths need. xmljson unmaintained.                                                                    |
| JSON Pointer + JSONPath engine      | Adopt — `python-jsonpath`         | One MIT dependency with no third-party requirements covers both RFC 6901 and RFC 9535. jsonpath-ng rejected on pre-standard dialect.                                                                                |
| YAML parser                         | Adopt — `ruamel.yaml`             | YAML 1.2 is a strict JSON superset, matching the "IR is a JSON value" contract exactly; avoids the Norway problem by specification. Governance risk accepted, bounded by the `Codec` port with `ruyaml` as drop-in. |
| CSV                                 | Adopt — stdlib `csv.DictReader`   | W3C csv2json minimal-mode output is already what `DictReader` produces — standards alignment is free.                                                                                                               |

### Scaffolder and remote templates

| Concern                                 | Decision                                                                            | Why                                                                                                                                                                                                                                              |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Template rendering                      | Build (thin) — stdlib `string.Template`                                             | Matches the no-evaluation requirement by construction where Jinja would have to be constrained into it; a generator dependency adds an install step to the one command whose purpose is removing friction. Considered copier, cookiecutter.      |
| Shipped command-line surface            | Adopt — stdlib `argparse`                                                           | Zero dependencies, so the scaffolder ships in the main package with `dependencies = []` untouched. (The cyclopts choice elsewhere is scoped to workshop tools and does not transfer.)                                                            |
| Filesystem write safety                 | Build (thin) — `tempfile` + `os.replace` + `Path.resolve().is_relative_to()`        | Narrow requirement — stage, verify, commit, never traverse out. A library would be more surface than the problem.                                                                                                                                |
| App installation entry (`spoc app`)     | Build/policy — print the exact config line; never edit config                       | Auto-editing needs comment-preserving TOML (tomlkit) — a runtime dep breaks `dependencies = []`; an extra forks behavior on installation state. Django-startapp parity.                                                                          |
| Archive member admission                | Adopt + Extend — stdlib PEP 706 filter plus our own containment re-verification     | The filter is the maintained standard, but CVE-2025-4517 was arbitrary write inside `filter="data"`, patched only above the supported floor; re-verifying each materialized path makes the CVE inert at any patch level.                         |
| Containment check                       | Extend — `resolve().is_relative_to()`                                               | Component-aware and immune to the common-prefix bug.                                                                                                                                                                                             |
| Resource bounds (expanded size)         | Build (thin) — streaming counter (~10 lines)                                        | No stdlib API bounds expanded size, and the only OSS candidate covers zip but not tar.                                                                                                                                                           |
| Retrieval transport and redirect policy | Adopt + Extend — stdlib transport + custom redirect handler                         | Transport is never hand-rolled; the zero-dependency invariant rules out httpx/requests for a shipped surface. Scheme-downgrade refusal is a dozen lines under any client.                                                                        |
| Reference grammar                       | Adopt — pip / PEP 508 direct-reference shape (`@ref`, `#subdirectory=`)             | The only published vocabulary expressing both an archive reference and a revision-pinned VCS reference; `gh:` is sugar over it, not a parallel scheme.                                                                                           |
| Cache location                          | Build (thin) — read `XDG_CACHE_HOME` / `LOCALAPPDATA` / `~/Library/Caches` directly | platformdirs is mature, but as a dependency it breaks the invariant and as an extra it reinstates the two-step install this feature exists to remove. Adopt the conventions, decline the wrapper.                                                |
| Origin record serialization             | Adopt — stdlib `json` (`.spoc-template.json`)                                       | The record interpolates arbitrary caller strings — that is serialization, not substitution, and the earlier TOML interpolation produced unparseable output for any Windows path. Supersedes the earlier "TOML writing — dissolved by scope".     |
| Origin record integrity                 | Build by construction — the operation authors the record                            | A description its subject can author is not a description. No rendering path reaches the record; a set declaring the reserved target is refused pre-write.                                                                                       |
| Keyword-safe decorator names            | Adopt — PEP 8 trailing underscore + stdlib `keyword.iskeyword`                      | The broken half is answered by a convention Python itself publishes; inflection libraries solve the half that is not broken.                                                                                                                     |
| Collisions introduced by escaping       | Build (thin) — append underscores until unused                                      | Escaping can create a collision the pre-escape check cannot see (`class` and `class_` both → `class_`). Total, terminates, degrades only toward importability.                                                                                   |
| Verifying platform-conditional behavior | Extend — platform becomes a value (pure function + thin adapter)                    | Every branch reachable from any host by construction. The mature pytest platform plugins all skip on the wrong platform — the categorical opposite of the requirement.                                                                           |
| Retention key for a revision            | Adopt + Extend — `hashlib`; verbatim when path-safe, else `rev-<sha256[:32]>`       | Filtering is lossy and lets two revisions share one entry. Later narrowed: no uppercase, no trailing dot, because NTFS/APFS fold case and strip trailing dots — the mapping must be injective under the host's equivalence, not string equality. |
| Evidence the retention key is injective | Adopt — Hypothesis property + explicit neighbourhood strategy                       | A claim over an open input domain; a real collision (`feature/x`) survived because no hand-picked example named it.                                                                                                                              |
| Provoking the concurrent-retention race | Adopt — pytest `monkeypatch` at the publish seam                                    | Deterministic, no sleeps, no scheduler dependence; two real threads with a barrier would be the one flaky test in the suite.                                                                                                                     |

### Testing, docs, and tooling

| Concern                                           | Decision                                                                           | Why                                                                                                                                                                                                                                                     |
| ------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Property-testing engine                           | Adopt — Hypothesis                                                                 | Shrinking, replay databases, and stateful exploration are subtle machinery, never hand-rolled. Dev group only.                                                                                                                                          |
| Isolation harness                                 | Build (thin) — on the kernel's own public API                                      | No OSS knows SPOC's boot lifecycle; a framework's test harness is inherently its own (django.test precedent).                                                                                                                                           |
| Fixture surfacing                                 | Adopt — pytest `pytest11` entry point                                              | Standard discovery, metadata-only at runtime — pytest never becomes a runtime dependency.                                                                                                                                                               |
| TOML generation in the harness                    | Adopt — `tomli-w`                                                                  | Standard-format serialization is never hand-rolled; already ships as the `toml` extra. Comment loss is unobservable because the scope restores original bytes on exit.                                                                                  |
| Dry-run isolation for diagnostics                 | Extend — our own `spoc.testing`                                                    | The isolation a dry boot needs already shipped as public API; duplicating it would be a coherence defect.                                                                                                                                               |
| Public API surface extraction and drift detection | Adopt — `griffe`                                                                   | The only option serving both needs — extraction feeds the drift check, and `griffe check` classifies breaking vs compatible change between refs. Dev dependency only.                                                                                   |
| Deprecation marking and runtime signal            | Extend — PEP 702 `warnings.deprecated` + a 3.12 fallback shim                      | The standard: a suppressible/escalatable `DeprecationWarning` plus static visibility. Stdlib only from 3.13 while the floor is 3.12; the shim deletes itself when the floor moves.                                                                      |
| Manifest parsing (for the surface checker)        | Adopt — stdlib `tomllib`                                                           | Costs nothing; `spoc.formats` explicitly rejected — the checker must not import the package whose surface it audits.                                                                                                                                    |
| Cross-release breaking-change detection           | Adopt — `griffe check`                                                             | The capability the griffe adoption already bought; verified running with `spoc` absent from the tool environment. AexPy rejected — it dynamically imports the target.                                                                                   |
| Static tier derivation                            | Extend — the existing griffe adoption                                              | Every fact the rules need is already on the extracted objects; reproduces all declared tiers with zero mismatches. Reverses the earlier tiers-as-manifest-data arrangement; the manifest keeps only non-import elements.                                |
| Deprecated re-export mechanism                    | Build (thin) — `deprecated_alias` on the adopted PEP 702 decorator                 | The signal must fire on the withdrawn path only and be visible to the tooling; PEP 562 attribute hooks are invisible to type checkers and griffe.                                                                                                       |
| Settling-condition detection                      | Extend — the adopted extractor                                                     | Project-specific vocabulary no general tool can know; griffe already parses the docstrings.                                                                                                                                                             |
| Reading the withdrawal mark from source           | Extend — griffe + a stdlib `ast` pass                                              | The prior assumption that griffe reads `__deprecated__` from source was wrong. Importing the package rejected outright: a checker that imports its subject audits whatever is installed.                                                                |
| Reconstructing per-release history                | Extend — the release-materializing adapter                                         | It already materializes a ref's `src/` for the ordinary extractor; a second way of reading a ref would be drift. Walked lazily — zero cost when nothing promised was removed.                                                                           |
| Doc snippet execution                             | Adopt — `pytest-examples`                                                          | Discovery via `find_examples()`, expected-output insertion via `--update-examples` — output is never typed by hand.                                                                                                                                     |
| API reference member lists                        | Adopt — mkdocstrings/griffe `__all__` derivation                                   | Drops hand-enumerated member lists; griffe treats `__all__` as authoritative.                                                                                                                                                                           |
| CLI reference generation                          | Extend — `mkdocs-macros` + a ~20-line help-dump macro over the real parser factory | Purpose-built plugins failed the maturity rubric.                                                                                                                                                                                                       |
| Starter template's surface stack                  | Adopt — stdlib, no third-party binding                                             | The kernel's pitch is surface-plural; a starter hardwiring one transport demotes every other. Bindings ship as executable docs recipes.                                                                                                                 |
| Settings-validation seam                          | Adopt — pydantic, as a worked example in docs only                                 | The seam receives an already-parsed table, so plain-model validation is the exact fit; the kernel imports nothing. pydantic-settings duplicates reading the kernel already did; dynaconf's layering competes with the mode cascade.                     |
| Coverage floor                                    | Adopt reporting, decline the gate — `coverage.py`, no `fail_under`                 | A floor cannot distinguish invariant lines from any others; it would make a number the target. Left open — revisit if erosion is observed.                                                                                                              |
| Type-reference extraction for stubs               | Build — on stdlib (`__module__`/`__qualname__`, `inspect.signature`)               | The describe pass holds live objects; every candidate reads source statically, which is blind to plugin components that exist only after configuration resolves.                                                                                        |
| Stub emission and formatting                      | Build + Adopt — our emitter; `ruff format` + PYI rules                             | No new dependency; stub linting comes from ruff's vendored flake8-pyi.                                                                                                                                                                                  |
| Stub conformance verification                     | Adopt — `assert_type` under mypy, pyright, and ty                                  | The promise is "a type checker resolves the promised type"; verifying with a checker no user runs verifies the wrong thing. stubtest rejected — it introspects the runtime and would flag deliberately runtime-absent types.                            |
| IDE autocomplete verification                     | Adopt — pyright as the proxy                                                       | Pylance is built on pyright, so pyright resolving the type is the evidence completion works.                                                                                                                                                            |
| Registry projection schema language               | Adopt — JSON Schema draft 2020-12, hand-written and checked in                     | The document is the format and the dataclass is one producer of it; a generator inverts that authority and cannot express the identifier pattern or an independent format version. Drift paid by a parity test, not by generation.                      |
| Validating the projection in the suite            | Adopt — `jsonschema`, dev group only                                               | Reference implementation, full 2020-12, pure Python. Reconciled with the earlier rejection: that was a runtime dep for internal config; this is a test dep for an external contract.                                                                    |
| A domain vocabulary for the projection            | None applies — JSON Schema is the whole adoption                                   | Schema.org describes software products, SPDX/CycloneDX describe dependency inventories, OpenAPI describes API surfaces; nothing standard describes what an application registered in-process. Revisit on a change of subject, not scale.                |
| Serializing the projection                        | Adopt — stdlib `json`                                                              | Never-hand-roll, and it holds the containment boundary — routing through `spoc.formats` would make an optional-extra subpackage load-bearing for a core surface.                                                                                        |
| Verifying concurrency in tests                    | Extend — existing pattern + `threading.Barrier`                                    | Test-only, so the runtime invariant does not bind; the stdlib primitive is what the free-threading guide recommends.                                                                                                                                    |
| Tiering the downstream command mount points       | Policy — `provisional`, all four                                                   | The signature takes the parsing library's private subparser type; promising it would commit SPOC and every downstream framework to that parser. `public` behind a SPOC-owned mount protocol is the durable answer, deferred until a real caller exists. |
| mypy strict over `src/`                           | Adopt — strict from day one                                                        | Already a pinned dev dependency, so no new ADR owed; strict immediately because the measured 34-error gap closed in one change.                                                                                                                         |

### Reversals and supersessions (final state)

- **Formats distribution:** split into `spoc-formats` under a uv workspace → reunified
  into the single `spoc` distribution, with the import boundary enforced by tests. A
  standing owner mandate makes one PyPI distribution non-negotiable.
- **TOML writing:** "dissolved by scope, no dependency" → superseded by the origin-record
  stdlib-`json` decision; separately, the test harness adopted `tomli-w`.
- **Stability tiers:** declared as data in `[tool.spoc.stability]` → derived from the
  artifact; the manifest keeps only the non-import elements griffe cannot see.
- **Name normalization:** "never called on the registration path" → reversed: derived
  names are converted then validated; stated names stay verbatim.
- **Reentrancy detection:** thread-identity owner field → folded into the `contextvars`
  membership marker and deleted.
- **Registry facet index:** a nested index beside the flat store → replaced by keying the
  single store on grammar segments, so "one store" is a structural guarantee rather than
  a procedural promise.
- **Per-object memo in the stubs manifest:** designed, implemented, then withdrawn — the
  registry's identity invariant makes the saving unreachable.
- **`apidiff` placement:** "not part of apicheck" → amended to a second console script in
  the same distribution, because computing additions requires the same tier rules.

Foreclosed adoptions worth remembering: `sortedcontainers` for registry facets and a
traversal/filtering library for `collect` — both refused on `dependencies = []`, with
stdlib `Path.walk` adopted instead of a hand-rolled recursive walk.

## Tech stack

Concrete choices, each tied to the decision that picked it (see the ADR tables above).

- **Language/runtime:** Python ≥ 3.12; the published package has **zero runtime
  dependencies** (`dependencies = []`). Kernel stdlib surface: `re`, `tomllib`,
  `graphlib`, `asyncio`, `inspect`, `threading.Lock`, `contextvars`, `logging`,
  `argparse`, `json`, `string.Template`, `tempfile`, `hashlib`, `keyword`.
- **Optional extras:** `yaml` → ruamel.yaml; `xml` → xmltodict; `toml` (write) → tomli-w;
  `query` → python-jsonpath (RFC 6901 + RFC 9535); `full` → all of them. CSV/JSON/TOML
  reading are stdlib.
- **Packaging:** hatchling build backend; uv for environments and lockfile;
  `hatch version` for version increments; one PyPI distribution, forever.
- **Testing:** pytest (integration via a `pytest11` entry point; never a runtime
  dependency), Hypothesis for property tests, coverage.py for reporting (no gate),
  jsonschema (dev-only) to validate the projection, pytest-examples to execute doc
  snippets.
- **Type checking:** mypy strict + ty as gating checkers; pyright additionally for stub
  conformance and as the IDE-autocomplete proxy. ruff for lint/format, including PYI
  rules for generated stubs.
- **API governance:** griffe under the in-repo `apicheck`/`apidiff` tools
  (`scripts/py/tools/`) for tier derivation, drift detection, cross-release breaking
  change classification, and deprecation-lifecycle enforcement; PEP 702
  `warnings.deprecated` (with a 3.12 shim) for withdrawal marking.
- **Docs:** MkDocs + mkdocstrings (griffe-backed member lists), mkdocs-macros for the
  generated CLI reference; pydantic appears in docs only as a worked settings-validation
  example.
- **CI:** the hosted CI platform, Linux + Windows + macOS × three Python versions, gate
  commands derived from `.canon/checks.md` via `Taskfile.yml` (`task check`).

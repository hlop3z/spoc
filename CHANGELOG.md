...
- **`--json` on `spoc check`, `spoc list`, and `spoc explain`.** The stability policy
  ...
  now. `check --json` emits `{format_version, ok, findings}` (versioned by
  `spoc.diagnostics.CHECK_FORMAT_VERSION`, and available to library callers as
  `CheckReport.to_dict()`, so the CLI and the library emit the identical document); `list --json` and `explain --json` emit the projection document's own

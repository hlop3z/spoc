...
A caller resolving a component by identifier can state what it expects that
component to be, and be told at access time when the registry holds something
of the wrong shape. The contract belongs to the caller, so obtaining a typed
reference never requires importing the application that declared it: the only
coupling is the canonical identifier.

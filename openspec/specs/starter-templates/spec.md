...
The starter template set turns `init` from a skeleton generator into an
application generator: one invocation yields a running, transport-neutral
project — the conventional vocabulary wired end to end, surfaces that are pure
registry projections, and no dependency beyond the kernel itself. It is held to
every contract ordinary template sets carry, chooses no transport for the
project, and leaves binding the projection to a transport as the project's own
few lines.

...
Which platforms the project's behavior is guaranteed on, and what makes that guarantee
evidence rather than assertion. Three things have to agree: what the project declares, what
the validation gate executes on, and what the documentation says. A platform claimed but not
gated is an untested claim; a platform gated but not declared is work nobody promised.

The capability also covers a subtler failure: behavior selected by platform is invisible to
whichever platform is not running the suite. Unless such behavior is verifiable from any host,
a contributor is blind to the platforms they do not use and a coverage measurement describes
the machine that produced it rather than the code it ran against.

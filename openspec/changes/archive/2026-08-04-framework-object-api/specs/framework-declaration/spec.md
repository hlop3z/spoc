...
follow the object-identity capability: validated, never normalized.

#### Scenario: Bare form

- **WHEN** an object whose own name already conforms to the identifier grammar is
  marked with the bare handle
- **THEN** it is registered under its own name

#### Scenario: Named form

- **WHEN** an object whose own name does not conform is marked with the handle and an
  explicit conforming name
- **THEN** it is registered under the explicit name, and omitting the explicit name
  fails with the invalid-segment error from the object-identity capability

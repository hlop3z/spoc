...
components into the registry) in one explicit start step that takes the project root. Discovery failures follow the
component-registry capability: loud, naming the object and reason. Starting an
already-started framework MUST fail rather than silently re-discover.

...
| Type checker      | `uv run ty check`                                      | Python (ty). `tests/conformance/` is excluded here on purpose — it is a standalone fixture project with its own import root, checked far harder by the row below. |
